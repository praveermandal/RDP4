...
async def check_session_validity(page, agent_id):
    ...
    if "login" in page.url:
        print(f"\n❌ [{agent_id}] SESSION EXPIRED. Check your INSTA_COOKIE!", flush=True)
        os._exit(1)
...
async def run_agent(agent_id, cookie, target_id, target_name):
    ...
    async with async_playwright() as p:
        # Launch Chromium with V100 optimized flags
        ...
        while (asyncio.get_event_loop().time() - start_time) < TOTAL_DURATION:
            try:
                ...
                # Using domcontentloaded to prevent "Stuck" behavior on network idle
                ...
                print(f"♻️ [{full_id}] Cycle end. Clearing Context...", flush=True)
                await context.close()
                gc.collect()
...
async def main():
    ...
    if not cookie or not target_id:
        print("❌ [CRITICAL] Missing GitHub Secrets (INSTA_COOKIE or TARGET_THREAD_ID)", flush=True)
        return

    print(f"💎 CLUSTER NODE {os.environ.get('MACHINE_NUMBER', '1')} INITIALIZED", flush=True)
